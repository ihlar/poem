...
informative:
  ...
  AltMark: RFC9341
  EFMP: I-D.mdt-quic-explicit-measurements

--- abstract

This document defines the Periodic On-path Explicit Measurement (POEM)
...
The Explicit Flow Measurement Protocol {{EFMP}} defines a per-packet
signaling approach based on the Alternate-Marking method {{AltMark}},
...
Marker packet:
: A POEM packet sent by an endpoint at a fixed interval that
  delineates measurement periods.

Report packet:
: A POEM packet sent by an endpoint that an on-path network element
  can update with measurement information.

Observer:
: An on-path network element that counts packets and/or writes
  measurement information into report packets.
...
POEM operates alongside QUIC connections. Endpoints negotiate POEM
support using a QUIC transport parameter ({{tp}}). Once negotiated,
...
- A 5-bit loss count reporting lost packets detected by the sender
  since the previous marker.

On-path observers detect marker packets by their version field and
...
Endpoints can also send report packets at a lower cadence. An
observer writes a 6-bit value into the report packet indicating the
fraction of observed loss that is upstream of its position.

{{fig-overview}} shows the basic operation of POEM.
...
+---+----+         +----+-----+           +----+-----+
    ...
    |                   |                      |
~~~
{: #fig-overview title="POEM Protocol Operation"}

Signals flow in the same direction as the data packets. The marker
...
so via application-layer mechanisms outside the scope of this document.
...
The POEM protocol does not carry any identifier or topology
...
POEM signals travel in the same direction as data. The receiver
obtains loss localization information but the sender does not, unless
the receiver communicates it back through application-layer means.
This is analogous to the model used by other on-path signaling
protocols such as SCONE {{SCONE}}.

## Per-Flow Scope

POEM measurement applies to a single flow identified by the UDP
4-tuple. Separate flows receive independent measurements, even if
they share the same endpoints or network path.


# Marker Packet {#marker}

A marker packet is a QUIC long header packet that follows the QUIC
invariants; see {{Section 5.1 of INVARIANTS}}.

{{fig-marker}} shows the format of the marker packet.
...
~~~
{: #fig-marker title="Marker Packet Format"}

The fields are defined as follows:

Header Form:
: The most significant bit (0x80) is set to 1, indicating a QUIC
  long header packet.

Reserved:
: The next bit (0x40) is reserved and can be set according to
  {{!QUIC-BIT=RFC9287}}.

Counter:
: A 1-bit alternating counter (0x20). The sender toggles this bit
  with each successive marker packet. This allows observers to detect
  lost markers and aids synchronization for observers joining mid-flow.

Loss Count:
: A 5-bit unsigned integer (0x1f) indicating the number of packets
  ...
  are detected in a single interval, the sender reports 31.

Version:
: A 32-bit version field set to 0xTBD1, identifying this as a POEM
  marker packet.

Destination Connection ID Length and Destination Connection ID:
: Set to the same value as other packets in the same datagram; see
  {{Section 12.2 of QUIC}}.

Source Connection ID Length and Source Connection ID:
: Set to match the Source Connection ID field of the packet that
  follows in the datagram. If the following packet has a short header,
  this field is empty and the length is set to 0.

Marker packets MUST be coalesced with other QUIC packets and MUST be
the first packet in the UDP datagram. A sender MUST NOT include more
than one marker packet in a datagram.


## Marker Interval

The marker interval is fixed at 64 packets. After sending a marker,
the sender counts 63 additional packets (including the QUIC packet
coalesced with the marker) and then sends the next marker.

The value 64 balances measurement granularity against overhead:
markers constitute less than 2% of total packets while individual
intervals yield useful loss counts at typical loss rates.
...
which may cause it to become negative.

When constructing a marker packet, the sender sets the loss count
...
A report packet is a QUIC long header packet that an on-path network
element can update with measurement information for the receiving
endpoint.

{{fig-report}} shows the format of the report packet.
...
~~~
{: #fig-report title="Report Packet Format"}
...
Upstream Ratio:
: A 6-bit unsigned integer (0x3f) representing the fraction of
  ...
  a value. When sent by the endpoint, this field MUST be set to 63.
  ...
  proportional fractions (value/62).

Version:
: A 32-bit version field set to 0xTBD2, identifying this as a POEM
  report packet.

Destination Connection ID and Source Connection ID:
: Same rules as for marker packets ({{marker}}).

Report packets MUST be coalesced with other QUIC packets and MUST
be the first packet in the UDP datagram.


## Upstream Ratio Semantics

The upstream ratio encodes the observer's estimate of what fraction
...
downstream and 62 means all loss is upstream.

The receiving endpoint combines this with its own end-to-end loss
...
downstream loss relative to the observer.

The observer computes this ratio from its own packet counting
(upstream loss) and the sender's reported loss count (end-to-end
loss). The computation is described in {{observer-behavior}}.


## Report Frequency

Report packets SHOULD be sent less frequently than marker packets.
A rate of one report every 4 to 8 marker intervals (every 256 to
512 packets) is RECOMMENDED. The precise cadence is a sender
decision.


# Sender Behavior {#sender-behavior}

A sender that has negotiated POEM support ({{tp}}) proceeds as follows:

1. Maintain a packet counter, initialized to 0 after each marker.

2. Maintain an Unreported Loss counter (persistent across markers),
...
3. After sending 63 packets following the previous marker, construct
   the next marker packet:
   - Toggle the counter bit.
   - Set the loss count field to min(Unreported_Loss_counter, 31)
     if positive, or 0 otherwise.
   - Subtract the reported value from the Unreported Loss counter.
   - Reset the packet counter.

4. Coalesce the marker packet as the first packet in the next UDP
   datagram, followed by one or more QUIC packets.

5. Periodically (at the chosen report cadence), send a report packet
   instead of or in addition to a marker packet, with the upstream
   ratio field set to 63.

POEM packets can only be sent once both endpoints have exchanged
the poem_supported transport parameter. This means the first POEM
packet cannot be coalesced with the Initial packets of a QUIC
handshake.

A sender SHOULD include a marker packet in one of the first few
datagrams sent after negotiation completes to allow observers to
begin measurement early.

A sender MUST NOT send both a marker packet and a report packet in
the same datagram.

## Connection Migration

When a QUIC connection migrates to a new path (new UDP 4-tuple),
the sender MUST reset its packet counter and MUST reset the
...
An observer is an on-path network element that reads and optionally
modifies POEM packets.

## Detecting POEM Packets

An observer identifies POEM packets by the long header bit (0x80) in
the first byte and the version field (0xTBD1 for markers, 0xTBD2 for
reports).

## Processing Marker Packets

An observer begins counting packets for a flow upon observing the
...
and to flows detected mid-way.

When an observer detects a marker packet, it:

1. Records the current packet count for the flow (identified by
   the UDP 4-tuple) since the last observed marker.

2. Reads the counter bit to detect marker loss. If the counter bit
...
3. Reads the loss count field, which indicates the sender's observed
   end-to-end loss for the preceding interval.

4. Computes upstream loss for the interval as:
   upstream_loss = N - observed_packet_count, where N is 64 (or
   128 if a lost marker was detected).

5. Forwards the marker packet unmodified.

The observer does not modify marker packets.

## Processing Report Packets

When an observer detects a report packet, it MAY write measurement
information into the upstream ratio field:

1. Compute the upstream loss rate from recent marker intervals.

2. Compute the total loss rate from the sender's reported loss counts
   over the same intervals.

3. If total loss is non-zero, compute:
   upstream_ratio = (upstream_loss / total_loss) * 62, clamped to
   the range 0-62.

4. If the current value in the upstream ratio field is 63 (not yet
   written by another observer), write the computed value.

The upstream loss (from packet counting) and the sender's reported
...
To account for this, observers SHOULD compute the upstream ratio
...
A receiver that has negotiated POEM support processes incoming
...
- The counter bit, to detect marker loss.
- The loss count, indicating the sender's reported losses.

The receiver MAY use this information for its own diagnostics or
relay it to the application layer.

A marker packet MUST be discarded (its contents ignored) unless
another QUIC packet in the same datagram is successfully processed.

## Processing Report Packets

The receiver reads the upstream ratio field. A value of 63 indicates
that no observer on the path wrote a measurement. Values 0 through 62
indicate an observer's estimate of the upstream loss fraction.

Combined with its own end-to-end loss measurement (from QUIC
acknowledgment processing), the receiver can estimate:

- Upstream loss rate = end_to_end_loss * (upstream_ratio / 62)
- Downstream loss rate = end_to_end_loss * (1 - upstream_ratio / 62)

The upstream ratio is a statistical estimate that converges over
time. Receivers SHOULD average the ratio over multiple reports
before drawing conclusions about loss localization.

The receiver has no information about which network element wrote
the ratio or where on the path it is positioned; see
{{applicability}}.

A report packet MUST be discarded unless another QUIC packet in
the same datagram is successfully processed.
...
How applications use this information is out of scope for this
document.


# Negotiating POEM {#tp}

A QUIC endpoint indicates willingness to receive POEM packets by
including the poem_supported transport parameter (0xTBD) in the
QUIC handshake.

The poem_supported transport parameter is empty. Receiving a
non-zero length value MUST be treated as a connection error of type
TRANSPORT_PARAMETER_ERROR.

Each endpoint independently indicates willingness to receive POEM
packets. An endpoint that does not include the transport parameter
can still send POEM packets if its peer has indicated support.

A client MUST NOT use a remembered value of poem_supported for 0-RTT
connections. POEM packets MUST NOT be sent until the transport
parameter is received from the peer.
...
POEM relies on on-path network elements modifying report packets.
...
model as SCONE {{SCONE}}.

## Fake POEM Packets

An attacker that can inject packets could construct fake POEM
...
Off-path attackers cannot construct valid QUIC packets.

## Misleading Reports

An on-path observer could write misleading upstream ratio values.
However, such an observer already has the ability to drop or delay
packets. Endpoints SHOULD treat POEM reports as advisory and
cross-reference with their own measurements.

## Interaction with Loss Detection

The loss count reveals information about the sender's loss detection
...
new exposure beyond what is available to on-path observers.

## Falsified Loss Signals

An endpoint can falsify POEM signals in two ways:

Inflated marker frequency:
: Sending markers more frequently than the specified interval of 64
  ...
  appear that 50% of packets are lost upstream.
...
POEM packets do not introduce new flow identifiers beyond what is
already visible in the UDP 4-tuple and QUIC connection IDs.

## Measurement Exposure

The loss count in marker packets reveals the sender's view of
...
## POEM Version Numbers

This document registers the following entries in the "QUIC Versions"
registry:
...
## poem_supported Transport Parameter

This document registers the following entry in the "QUIC Transport
Parameters" registry:
...
detection of changes in loss patterns.

Future versions of this protocol may allow endpoints to select from
...
use simple packet counting rather than per-packet bit tracking.

POEM is suited to deployments where statistical loss estimates over
multiple intervals are sufficient.


# Acknowledgments
{:numbered="false"}

This work is inspired by the explicit measurement techniques defined
in {{EXPLICIT-MEASUREMENTS}} and the architectural approach of the
SCONE protocol {{SCONE}}.
