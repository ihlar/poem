...
abbrev: "POEM"
docname: draft-ihlar-poem-quic-latest
category: info
...
so via mechanisms outside the scope of this document.
...
For the purpose of POEM, a packet is a single UDP datagram.
Observers count UDP datagrams on the 4-tuple regardless of how many
QUIC packets are coalesced within each datagram.
...
directly in the marker's loss count field.

The counter is incremented each time QUIC's loss detection declares
...
## Sending Marker Packets {#sending-markers}

POEM packets can only be sent once both endpoints have exchanged
the poem_supported transport parameter ({{tp}}). This means the
...
## Observing Marker Packets {#observing-markers}

An observer identifies marker packets by the long header bit (0x80)
in the first byte and the version field 0xTBD1.
...
1. Records the current packet count for the flow (identified by
   the UDP 4-tuple) since and including the previous marker.
...
## Receiving Marker Packets {#receiving-markers}
...
## Sending Report Packets {#sending-reports}
...
When sending a report packet, the sender sets the upstream ratio
...
## Observing Report Packets {#observing-reports}

An observer identifies report packets by the long header bit (0x80)
in the first byte and the version field 0xTBD2.
...
4. Write the computed value into the upstream ratio field.
...
## Receiving Report Packets {#receiving-reports}
...
Combined with its own end-to-end loss measurement, the receiver can
estimate:

- Upstream loss rate = end_to_end_loss * (upstream_ratio / 62)
- Downstream loss rate = end_to_end_loss * (1 - upstream_ratio / 62)

The receiver can derive end-to-end loss either from its own
...
an independent measurement that does not depend on ACK processing.
...
however, and endpoints SHOULD NOT assume a particular observer
location.
...
A QUIC endpoint indicates that it is able to receive POEM packets by
...
The poem_supported transport parameter MUST be empty. Receiving a
non-zero length value MUST be treated as a connection error of type
TRANSPORT_PARAMETER_ERROR.

Endpoints MUST NOT remember whether the poem_supported transport
...
Deflated loss count:
: Reporting a lower loss count than actually observed (including
  always reporting 0) causes the observer to attribute all observed
  loss to the upstream segment, concealing downstream loss.
